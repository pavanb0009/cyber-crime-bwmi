...
export function classifyEvidenceName(name) {
    const text = name.toLowerCase();
    if (/(upi|transaction|payment|bank|receipt)/.test(text))
        return { name, kind: 'Transaction proof', confidence: 96 };
    if (/(whatsapp|telegram|chat|message)/.test(text))
        return { name, kind: 'Chat screenshot', confidence: 94 };
    if (/(profile|account|instagram|facebook|suspect)/.test(text))
        return { name, kind: 'Suspect profile', confidence: 91 };
    if (/\.(pdf)$/i.test(name))
        return { name, kind: 'Document / statement', confidence: 86 };
    if (/\.(png|jpe?g|webp)$/i.test(name))
        return { name, kind: 'Screenshot / image', confidence: 82 };
    if (/\.(txt)$/i.test(name))
        return { name, kind: 'Text export', confidence: 78 };
    return { name, kind: 'Other evidence', confidence: 70 };
}
export function deriveEvidenceItems(names) {
    return names.map(classifyEvidenceName);
}
export function extractEvidence(draft) {
...
export function evidenceCompleteness(draft) {
    const present = [];
    const missing = [];
...
export function getActionPlan(type) {
    const plans = {
        financial: [
...
export function buildIncidentTimeline(draft) {
    const base = draft.occurredAt ? new Date(draft.occurredAt) : new Date();
    const validBase = Number.isNaN(base.getTime()) ? new Date() : base;
    const fmt = (date) => new Intl.DateTimeFormat('en-IN', { hour: '2-digit', minute: '2-digit' }).format(date);
    const add = (minutes) => new Date(validBase.getTime() + minutes * 60_000);
    const items = [];
    ...
    if (draft.incidentType === 'financial' && draft.amount) {
        ...
        if (draft.transactionId)
            items.push({ time: fmt(add(17)), event: `Transaction ${draft.transactionId} recorded` });
    }
    if (draft.incidentType === 'account') {
        items.push({ time: fmt(add(10)), event: `${draft.accountPlatform || 'Account'} access issue identified` });
        if (draft.contactChanged === 'yes')
            items.push({ time: fmt(add(18)), event: 'Linked email/phone change reported' });
    }
    if (draft.evidenceItems.length)
        items.push({ time: fmt(add(25)), event: `${draft.evidenceItems.length} evidence item${draft.evidenceItems.length === 1 ? '' : 's'} preserved` });
