import type { CopilotResult, IdentifierType } from '../types'
import { classifyIncident } from './intelligence'

export type CitizenDestination = 'emergency' | 'report' | 'check' | 'call' | 'notice'

export type CitizenIntent = {
  destination: CitizenDestination
  destinationLabel: string
  actionLabel: string
  url: string
  result: CopilotResult
  extracted?: { type: IdentifierType; value: string }
}
...
function firstIdentifier(text: string): StoryDetails {
  ...
  const amount = numericAmount
    ?? (scaledAmount
      ? String(
          Math.round(
            Number(scaledAmount[1].replace(/,/g, ''))
  ...
  if (phone) return { amount, transactionId, identifier: phone, identifierType: 'phone', channel: 'Phone call / SMS' }
  if (atValue) {
    return {
      amount,
      transactionId,
  ...
  if (url) return { amount, transactionId, identifier: url, identifierType: 'url', channel: 'Website / mobile app' }
  return { amount, transactionId }
}

export function extractStoryDetails(input: string): StoryDetails {
  const details = firstIdentifier(input)
  const text = input.toLowerCase()
  if (!details.channel) {
    if (/(whatsapp|telegram)/.test(text)) details.channel = 'WhatsApp / Telegram'
    else if (/(instagram|facebook|social)/.test(text)) details.channel = 'Instagram / Facebook / social media'
    else if (/(email|mail)/.test(text)) details.channel = 'Email'
    else if (/(website|link|url|app)/.test(text)) details.channel = 'Website / mobile app'
    else if (/(call|phone|sms|message)/.test(text)) details.channel = 'Phone call / SMS'
  }
  return details
}
...
export function resolveCitizenIntent(input: string): CitizenIntent {
