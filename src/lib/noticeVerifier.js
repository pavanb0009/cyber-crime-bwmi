...
function detectPaymentDemand(text) {
...
export function extractNoticeFields(text) {
    ...
    const referenceNumber = text.match(/(?:ref(?:erence)?|notice|case|complaint|file|f\.?\s*no|diary|fir|din)\s*(?:no\.?|number|#|:)?\s*[:\-#]?\s*([A-Z0-9][A-Z0-9/\-.]{4,})/i)?.[1]?.trim() ??
        text.match(/\b([A-Z]{2,}[/-]\d{2,}[/-][A-Z0-9/-]{2,})\b/)?.[1]?.trim() ??
        null;
    const officer = text.match(/(?:officer|inspector|i\.?o\.?|sho|dsp|acp|dcp|superintendent|investigating officer|signed by|authori[sz]ed by)\s*[:\-]?\s*((?:(?:Mr|Ms|Shri|Smt|Dr)\.?\s*)?[A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2})/)?.[1]?.trim() ??
        null;
    const phones = uniq([
    ...
    const links = uniq((text.match(/(?:https?:\/\/)?(?:[\w-]+\.)+[a-z]{2,}(?:\/[\w\-./?%&=+#]*)?/gi) ?? [])
        .filter((l) => !l.includes('@')) // drop emails caught by the URL regex
        .filter((l) => /\.[a-z]{2,}/i.test(l)));
    const paymentHits = detectPaymentDemand(text);
    const amount = text.match(/(?:₹|Rs\.?|INR)\s?([\d,]+(?:\.\d+)?)(?:\s?(?:lakh|lakhs|crore|thousand|k))?/i)?.[0]?.trim() ??
        null;
...
// --- Analysis ---------------------------------------------------------------
export function analyseNoticeText(rawText, fileName = '') {
    const text = `${rawText}\n${fileName}`.trim();
    ...
    let verdict;
    if (score >= 55 || dangerCount >= 2)
        verdict = 'high-risk';
    else if (score >= 25 || dangerCount >= 1)
        verdict = 'suspicious';
    else
        verdict = 'genuine-format';
    const headline = verdict === 'high-risk'
    ...
    const recommendedActions = verdict === 'genuine-format'
        ? [
    ...
    return {
        ...
        recommendedActions,
        disclaimer: 'Prototype heuristic checker. It detects scam signals in the notice text and cannot confirm that any notice is genuine. Always verify through an official government channel.',
    };
}
